...
public class Proposition extends Entity{
	...
	public boolean checkForValidSupport(Context context) {
		for (Iterator<Support> iterator = this.originSupport.iterator(); iterator
			...
			if (context.getHypothesisSet().propositions
					.containsAll(support.getOriginSet().getHypothesisSet().propositions)
					&& !support.getOriginSet().getHypothesisSet().propositions.isEmpty()) {
				return true;
			}
		}
		return false;
	...
	public void addSupport(Support support) {
		...
		this.updated = false;
		for (Iterator<PropositionNode> iterator = support.getOriginSet().getHypothesisSet().propositions
...
//			for (Iterator<Support> iterator1 = this.justificationSet.iterator(); iterator1
//					.hasNext();) {
//				HashSet<Support> temp = new HashSet<Support>();
//				Support s1 = iterator1.next();
//
//				for (Iterator<PropositionNode> iterator2 = s1.originSet.hypothesisSet.propositions
...
//	}
	...
	private HashSet<Support> multiply(HashSet<Support> prop1,
		...
		Support temp1 = new Support();
		Support temp2 = new Support();
		if (prop1.isEmpty())
			return prop2;
	...
	public HashSet<Support> calcOrigin(PropositionSet origins){
		if(origins.propositions.size()>2){
			...
			Iterator<PropositionNode> iterator1 = origins.propositions.iterator();
